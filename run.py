...
# For some reason it fixes windowing issues though so YAY!
#--------------------
class BenWindow(ui.window.BaseWindow):
    def create_window(self, params):
        ...
        super(BenWindow, self).create_window(params)
    ...
    def on_keyboard(self, key, scancode=None, unicode=None):
        if key == 27:
            ...
            return True
        super(BenWindow, self).on_keyboard(key, scancode, unicode)

    def flip(self):
        pygame.display.flip()
        super(BenWindow, self).flip()
    ...
    def _set_size(self, size):
        ...
        if super(BenWindow, self)._set_size(size):
            self._pygame_set_mode()
...
v = ParticleVisualisation(width = w.width, height = w.height, stream=stream)
w.add_widget(v)
